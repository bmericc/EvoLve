...
function evolve_discussion_title( $type = NULL, $echo = true ) {
  ...
  $discussion_title = '';
	...
	switch( $type ) {
		case 'comment' :
			$count = $comment_count;
			$many  = apply_filters( 'evolve_many_comments', (string) '% Comments' ); // Available filter: evolve_many_comments
			$none  = apply_filters( 'evolve_no_comments', (string) 'No Comments Yet' ); // Available filter: evolve_no_comments
			$one   = apply_filters( 'evolve_one_comment', (string) '1 Comment' ); // Available filter: evolve_one_comment
			break;
		case 'pings' :
			$count = $ping_count;
			$many  = apply_filters( 'evolve_many_pings', (string) '% Trackbacks' ); // Available filter: evolve_many_pings
			$none  = apply_filters( 'evolve_no_pings', (string) '' ); // Available filter: evolve_no_pings
			$one   = apply_filters( 'evolve_one_ping', (string) '1 Trackback' ); // Available filter: evolve_one_comment
			break;
	}
